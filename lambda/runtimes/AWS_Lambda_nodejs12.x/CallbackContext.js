...
function _rawCallbackContext(client, id, scheduleNext) {
	...
	const complete = (result, callback) => { client.postInvocationResponse(result, id, callback); };
		...
		if (err !== undefined && err !== null) {
			...
			if (!waitForEmptyEventLoop) {
				...
				BeforeExitListener.set(() => {
					setImmediate(() => { complete(result, scheduleNext); });
				});
			}
		}
	};
	...
	let succeed = result => { done(null, result); };
	let fail = err => {
		if (err === undefined || err === null) {
			done("handled");
		} else {
	...
	let callbackContext = {
		...
		set callbackWaitsForEmptyEventLoop(value) { waitForEmptyEventLoop = value; },
		succeed: succeed,
		fail: fail,
		done: done
	};
