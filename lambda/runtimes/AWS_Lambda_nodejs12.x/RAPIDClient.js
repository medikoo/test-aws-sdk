...
module.exports = class RAPIDClient {
	constructor(hostnamePort, httpClient, nativeClient) {
		...
		this.nativeClient = nativeClient || require("/var/runtime/build/Release/rapid-client.node");
		...
		this.agent = new this.http.Agent({ keepAlive: true, maxSockets: 1 });
	}
	...
	postInitError(error, callback) {
		...
		this._post(
			`/2018-06-01/runtime/init/error`, response, { [ERROR_TYPE_HEADER]: response.errorType },
			callback
		);
	}
			...
			return new Promise((resolve, reject) => {
				let request = this.http.request(options, response => {
					...
					response
						...
						.on("end", () => {
							resolve({ bodyJson: data, headers: response.headers });
						});
				});
				request.on("error", e => { reject(e); }).end();
			});
		}
	...
	_post(path, body, headers, callback) {
		...
		request.on("error", e => { throw e; }).end(bodyString, "utf-8");
	}
};
