...
module.exports.formatted = err => {
	try { return JSON.stringify(new XRayFormattedCause(err)); }
	catch (err) { return ""; }
};
	...
	constructor(err) {
		...
		if (err.stack) {
			...
			stackLines.forEach(stackLine => {
				...
				let label =
					line.lastIndexOf(" ") >= 0 ? line.slice(0, line.lastIndexOf(" ")) : null;
				let path =
				...
				let entry = { path: path[0], line: parseInt(path[1]), label: label || "anonymous" };

				stack.push(entry);
			});
		}

		this.exceptions = [{ type: err.name, message: err.message, stack: stack }];

		let paths = new Set();
		stack.forEach(entry => { paths.add(entry.path); });
