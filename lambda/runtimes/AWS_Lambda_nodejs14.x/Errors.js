...
function toRapidResponse(error) {
	try {
		if (util.types.isNativeError(error) || _isError(error)) {
			...
			return { errorType: typeof error, errorMessage: error.toString(), trace: [] };
		}
	} catch (_err) {
		return {
			errorType: "handled",
...
module.exports.toFormatted = error => {
	try { return "\t" + JSON.stringify(error, (_k, v) => _withEnumerableProperties(v)); }
	catch (err) { return "\t" + JSON.stringify(toRapidResponse(error)); }
};
...
function _withEnumerableProperties(error) {
	if (error instanceof Error) {
		let ret = Object.assign(
			{ errorType: error.name, errorMessage: error.message, code: error.code }, error
		);
