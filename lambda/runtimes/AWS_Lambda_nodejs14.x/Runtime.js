...
module.exports = class Runtime {
	...
	async handleOnce() {
		...
		let [callback, callbackContext] = CallbackContext.build(
			this.client, invokeContext.invokeId, this.scheduleIteration.bind(this)
		);
		...
		try {
			...
			let result = this.handler(
				JSON.parse(bodyJson), invokeContext.attachEnvironmentData(callbackContext), callback
			);
	...
	_setErrorCallbacks(invokeId) {
		this.errorCallbacks.uncaughtException = error => {
			this.client.postInvocationError(error, invokeId, () => { process.exit(129); });
		};
		this.errorCallbacks.unhandledRejection = error => {
			this.client.postInvocationError(error, invokeId, () => { process.exit(128); });
		};
	}
...
function _isPromise(obj) { return obj && obj.then && typeof obj.then === "function"; }
